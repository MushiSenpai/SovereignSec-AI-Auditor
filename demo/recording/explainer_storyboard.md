...
| 4 | 32–46s | Play `ablation.gif`. Highlight: **Semgrep alone R=0.5 → +cross-file taint P=R=1.0**. | "Off-the-shelf scanners miss cross-file bugs — here a SQL injection that flows through three files. Our taint engine catches it, with no false positives on safe code." |
